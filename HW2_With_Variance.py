...
class MPPIRacetrack:
    ...
    def compute_rollouts(self, initial_state: np.ndarray, goal_pos: np.ndarray):
        ...
        # delta_v = np.random.normal(0.9,1,(self.n,int(self.T/self.timestep)))
        # delta_v = np.zeros((self.n,int(self.T/self.timestep))) + 1.0
        # delta_w = np.random.normal(0,2.5,(self.n,int(self.T/self.timestep)))
        scale_w = 2.5*np.tanh(0.7*(np.abs(goal_pos[0] - initial_state[0]) + np.abs(goal_pos[1] - initial_state[1])))
        scale_v = 0.25*np.tanh(0.1*(np.abs(goal_pos[0] - initial_state[0]) + np.abs(goal_pos[1] - initial_state[1])))
        # if scale<1:
        #     scale = 1
        print("Scale_w: ",scale_w)
        print("Scale_v: ",scale_v)
        delta_v = np.random.normal(1, scale_v, (self.n, int(self.T / self.timestep)))
        delta_w = np.random.normal(0,scale_w,(self.n,int(self.T/self.timestep)))
        # print(delta_v)
        # print(delta_w)
        action = np.zeros(2)
    ...
    def score_rollouts(self, rollout_states, delta_v, delta_w, initial_state: np.array, goal_pos: np.ndarray):
        ...
        for i in range(self.n):
          ...
          # rollout_scores[i] += x_diff_goal + y_diff_goal
...
def run_planner_on_racetrack(
    ...
    plt.title("Unicycle on Racetrack (DV-MPPI)")
    plt.show()
